...
class Gamification
  ...
  def extra_attendances_in_month(attendance)
    ...
    attendances_in_month = @context[:attendances].reject do |a|
      a.occurrence.start
...
# find all occurrences in the month
    occurrences_in_month = @context[:occurrences].reject do |o|
    ...
    @points += 2 if occurrences_in_month.all? do |o|
      @context[:attendances].any? { |a| a.occurrence.start == o.start }
    end
